...
/**
 * Test the visibility of nodes that are handled by OG access.
 *
 * @group og_access
 */
class OgAccessTestCase extends \Drupal\simpletest\WebTestBase {
  ...
  public function setUp() {
    ...
    $settings[OG_ACCESS_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['value'] = 1;
  ...
  function testSubGroupContentAccess() {
    ...
    og_group('node', $this->group_node->nid, [
      'entity' => $this->user2
      ]);
  ...
  function testGroupContentAccessDefault() {
    ...
    $settings[OG_AUDIENCE_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['target_id'] = $nid;
    $settings[OG_CONTENT_ACCESS_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['value'] = OG_CONTENT_ACCESS_DEFAULT;
  ...
  function testGroupContentAccessNotDefault() {
    ...
    foreach ([
      ...
      $settings['type'] = $this->group_type;
      $settings[OG_GROUP_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['value'] = 1;
      $settings[OG_ACCESS_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['value'] = $state;
      ...
      $settings['type'] = $this->group_content_type;
      $settings[OG_AUDIENCE_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['target_id'] = $nid;
      $settings[OG_CONTENT_ACCESS_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['value'] = OG_CONTENT_ACCESS_PUBLIC;
      $public_node = $this->drupalCreateNode($settings);

      // Create a group content node and set private access.
      $settings[OG_CONTENT_ACCESS_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['value'] = OG_CONTENT_ACCESS_PRIVATE;
  ...
  function testOgStrictPrivate() {
    ...
    $settings[OG_ACCESS_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['value'] = 0;
    $group_node2 = $this->drupalCreateNode($settings);

    // Create a group content node and set default access.
    $settings = [];
    $settings['type'] = $this->group_content_type;
    $settings[OG_AUDIENCE_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['target_id'] = $group_node1->nid;
    $settings[OG_AUDIENCE_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][1]['target_id'] = $group_node2->nid;
    $settings[OG_CONTENT_ACCESS_FIELD][\Drupal\Core\Language\Language::LANGCODE_NOT_SPECIFIED][0]['value'] = OG_CONTENT_ACCESS_DEFAULT;
    ...
    $records = db_query('SELECT realm, gid FROM {node_access} WHERE nid = :nid', [
      ':nid' => $node->nid
      ])->fetchAll();
